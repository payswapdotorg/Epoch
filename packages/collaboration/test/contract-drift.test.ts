// Contract drift: committed artifacts under packages/collaboration/schemas
...
import { renderCollaborationContractFiles, COLLABORATION_SCHEMA_SURFACE } from '../src/index';

const here = path.dirname(fileURLToPath(import.meta.url));
const CONTRACTS_DIR = path.resolve(here, '..', 'schemas');
const UPDATE_MODE = process.env.EPOCH_UPDATE_CONTRACTS === '1';

describe('packages/collaboration/schemas drift', () => {
  const rendered = renderCollaborationContractFiles();
  ...
  it('renders a manifest plus one schema file per surface type', () => {
    ...
    expect(schemaFiles).toHaveLength(COLLABORATION_SCHEMA_SURFACE.length);
  });
  ...
  it.each(Object.keys(rendered).sort())('committed artifact %s matches emission', (rel) => {
    ...
    expect(
      existsSync(target),
      `missing committed artifact: packages/collaboration/schemas/${rel}`,
    ).toBe(true);
    expect(readFileSync(target, 'utf8')).toBe(rendered[rel]!);
  });

  it('emission is deterministic (two renders are byte-identical)', () => {
    expect(renderCollaborationContractFiles()).toEqual(rendered);
  });
  ...
  it('every emitted schema declares a versioned $id and draft 2020-12', () => {
    for (const [rel, content] of Object.entries(rendered)) {
      ...
      expect(parsed.$id, rel).toMatch(/^urn:epoch:collaboration:[a-z0-9-]+:1\.0\.0$/);
    }
  });
});
