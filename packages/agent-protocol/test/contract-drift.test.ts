// Contract drift: the committed artifacts under contracts/agent must be
// byte-identical to what the implementation emits. Regeneration is only
...
describe('contracts/agent drift', () => {
  ...
  it('renders a manifest plus one schema file per surface type', () => {
    ...
    expect(paths.filter((p) => p.startsWith('schemas/'))).toHaveLength(
      paths.length - 1,
    );
  });
  ...
  it.each(Object.keys(rendered).sort())('committed artifact %s matches emission', (rel) => {
    ...
    expect(existsSync(target), `missing committed artifact: contracts/agent/${rel}`).toBe(
      true,
    );
  ...
  it('emission is deterministic (two renders are byte-identical)', () => {
    expect(renderAgentContractFiles()).toEqual(rendered);
  });
});
