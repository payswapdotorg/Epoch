 ...
 * `packages/identity/schemas/` (same policy as @epoch/capability-registry,
...
export function renderIdentityContractFiles(): Readonly<Record<string, string>> {
  ...
  const manifest = {
    ...
    description:
      'Typed, versioned, provider-neutral Identity contract surface (W009): principals and principal ids, credential-assertion descriptors (mechanism classes, never secrets), and authentication-result records with typed failure reasons. Zero concrete IdPs, zero network, zero secrets storage. JSON Schema projection under schemas/; runtime validators in @epoch/identity.',
    dataTypes: IDENTITY_SCHEMA_SURFACE.map((entry) => entry.type),
    jsonSchemaFidelity:
      'structural-only: zod refinements (failed results carry exactly one reason, verified results carry none) are enforced by the runtime validators in @epoch/identity and are not represented in the JSON Schema files',
    emittedBy:
