/**
 * Semantic-version machinery for the Adapter SDK (self-contained).
 *
 ...
 *   exactly `0.0.p`.
 *
 * DELIBERATE DUPLICATION (Tech Lead pin): this module is byte-equivalent
 * to the one in @epoch/capability-registry because W007's two packages
 * share NO runtime coupling beyond @epoch/agent-protocol (the only
 * permitted @epoch runtime dependency). Cross-package drift is pinned by
 * the semver parity test in this package's test suite (devDependency
 * precedent: W006 evidence -> W002 world-model), which runs both
 * implementations over a shared corpus and asserts identical
