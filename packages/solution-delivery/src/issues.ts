/**
 * Flattened-issue helpers shared by the solution-delivery total entry
 * points — zod issues become typed {@link DeliveryIssue} values with
 * precise dotted paths (the W006/W007/W010/W023 issue style), plus the
 * vendor-field classifier that turns strict-object `unrecognized_keys`
 * rejections into the typed `vendor-fields-rejected` error.
 */
import type { ZodError } from 'zod';
import type { DeliveryError, DeliveryIssue } from './errors';
...
export function validationError(error: ZodError): DeliveryError {
  const issues = flattenZodIssues(error);
  return {
    code: 'validation',
    message: `solution-delivery document failed schema validation (${issues.length} issue${issues.length === 1 ? '' : 's'})`,
    issues,
  };
}
 ...
 * attempted to enter a solution-delivery record.
 */
...
export function vendorFieldsError(error: ZodError): DeliveryError {
  const issues = flattenZodIssues(error);
  return {
    code: 'vendor-fields-rejected',
    message:
      'record carries unknown structural fields — provider/vendor fields cannot enter solution-delivery records ' +
      '(strict objects; adapterize provider semantics behind the external request/event seam instead)',
    issues,
