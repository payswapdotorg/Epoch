// Contract drift: the committed artifacts under the in-package
// contracts/ directory must be byte-identical to what the implementation
// emits. Regeneration is only possible through the documented update
// mode, so artifacts can never drift silently from the schemas.
...
describe('contracts (in-package) drift', () => {
  ...
  it('emission is deterministic (two renders are byte-identical)', () => {
    expect(renderEvaluationContractFiles()).toEqual(rendered);
  });
});
