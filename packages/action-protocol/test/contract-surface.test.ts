...
import { ACTION_PROTOCOL_SCHEMA_SURFACE } from '../src/surface';
import {
...
const surfaceTypes = ACTION_PROTOCOL_SCHEMA_SURFACE.map((entry) => entry.type);

/**
 * Shared primitives mirrored from contracts/agent so this contract surface
 * is self-contained (documented in contracts/actions/README.md). They are
 * allowed as extra exports of index.d.ts and as extra parity assertions.
 */
const MIRRORED_PRIMITIVES = ['AgentId', 'JsonValue', 'MessageId', 'Timestamp'];
...
describe('contracts/actions surface integrity', () => {
  it.skipIf(UPDATE_MODE)('manifest dataTypes equal the package schema surface', () => {
    ...
    expect(manifest.contract).toBe('epoch/actions');
    expect(manifest.contractVersion).toBe(ACTION_CONTRACT_VERSION);
    expect(manifest.protocolVersion).toBe(ACTION_PROTOCOL_VERSION);
    expect(manifest.messageKinds).toEqual([...ACTION_PROTOCOL_MESSAGE_KINDS]);
