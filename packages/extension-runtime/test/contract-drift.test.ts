// Contract drift: committed artifacts under packages/extension-runtime/schemas
...
describe('packages/extension-runtime/schemas drift', () => {
  const rendered = renderExtensionRuntimeContractFiles();
  ...
  it('renders a manifest plus one schema file per surface type', () => {
    ...
    expect(schemaFiles).toHaveLength(EXTENSION_RUNTIME_SCHEMA_SURFACE.length);
  });
  ...
  it.each(Object.keys(rendered).sort())('committed artifact %s matches emission', (rel) => {
    ...
    expect(
      existsSync(target),
      `missing committed artifact: packages/extension-runtime/schemas/${rel}`,
    ).toBe(true);
    expect(readFileSync(target, 'utf8')).toBe(rendered[rel]!);
  });

  it('emission is deterministic (two renders are byte-identical)', () => {
    expect(renderExtensionRuntimeContractFiles()).toEqual(rendered);
  });
  ...
  it('every emitted schema declares a versioned $id and draft 2020-12', () => {
    for (const [rel, content] of Object.entries(rendered)) {
      ...
      expect(parsed.$id, rel).toMatch(/^urn:epoch:extension-runtime:[a-z0-9-]+:1\.0\.0$/);
    }
  });
