// Contract drift: the committed artifacts under the in-package contracts/
// directory must be byte-identical to what the implementation emits.
...
describe('contracts (in-package) drift', () => {
  ...
  it('emission is deterministic (two renders are byte-identical)', () => {
    expect(renderSimulationContractFiles()).toEqual(rendered);
  });
});
