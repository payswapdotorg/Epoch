...
import { SIMULATION_PROTOCOL_SCHEMA_SURFACE } from '../src/surface';
import {
...
const surfaceTypes = SIMULATION_PROTOCOL_SCHEMA_SURFACE.map((entry) => entry.type);
...
describe('simulation contracts surface integrity', () => {
  it.skipIf(UPDATE_MODE)('manifest dataTypes equal the package schema surface', () => {
    ...
    expect(manifest.contract).toBe('epoch/simulation');
    expect(manifest.contractVersion).toBe(SIMULATION_CONTRACT_VERSION);
    expect(manifest.protocolVersion).toBe(SIMULATION_PROTOCOL_VERSION);
    expect(manifest.messageKinds).toEqual([...SIMULATION_PROTOCOL_MESSAGE_KINDS]);
