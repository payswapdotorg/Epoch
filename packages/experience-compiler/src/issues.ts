/**
 * Flattened-issue helpers for the compiler's total entry points — zod
 * issues become typed {@link CompilerIssue} values with precise dotted
 * paths (the W006/W007/W008 issue style, mirrored from the W011
 * experience-protocol discipline).
 */
import type { ZodError } from 'zod';
import type { CompilerError, CompilerIssue } from './errors';
 ...
 */
export function flattenZodIssues(error: ZodError): CompilerIssue[] {
...
export function malformedDescriptorError(error: ZodError): Extract<
  CompilerError,
  { readonly code: 'malformed-descriptor' }
> {
  const issues = flattenZodIssues(error);
  return {
    code: 'malformed-descriptor',
    message: `experience-compiler input failed schema validation (${issues.length} issue${
      issues.length === 1 ? '' : 's'
    })`,
    issues,
  };
