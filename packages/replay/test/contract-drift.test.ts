// Contract drift: committed artifacts under packages/replay/schemas must
...
import { renderReplayContractFiles, REPLAY_SCHEMA_SURFACE } from '../src/index';

const here = path.dirname(fileURLToPath(import.meta.url));
const CONTRACTS_DIR = path.resolve(here, '..', 'schemas');
const UPDATE_MODE = process.env.EPOCH_UPDATE_CONTRACTS === '1';

describe('packages/replay/schemas drift', () => {
  const rendered = renderReplayContractFiles();
  ...
  it('renders a manifest plus one schema file per surface type', () => {
    ...
    expect(schemaFiles).toHaveLength(REPLAY_SCHEMA_SURFACE.length);
  });
  ...
  it('every emitted schema declares a versioned $id and draft 2020-12', () => {
    for (const [rel, content] of Object.entries(rendered)) {
      ...
      expect(parsed.$id, rel).toMatch(/^urn:epoch:replay:[a-z0-9-]+:1\.0\.0$/);
    }
  });
});
