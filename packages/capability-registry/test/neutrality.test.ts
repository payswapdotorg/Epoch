...
// framework, model, or API surface (architecture lock rule 13).
import { describe, expect, it } from 'vitest';
import { renderCapabilityRegistryContractFiles } from '../src/index';
...
describe('capability-registry contract provider neutrality', () => {
  ...
  it('the trust surface names origin classes, never vendors', () => {
    const rendered = renderCapabilityRegistryContractFiles();
    const origin = JSON.parse(rendered['capability-origin.schema.json']!) as {
      $defs: Record<string, { enum?: string[] }>;
    };
    expect(origin.$defs['CapabilityOrigin']?.enum).toEqual([
      'first-party',
      'community',
      'external-software',
