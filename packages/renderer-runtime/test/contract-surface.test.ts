...
import {
  RENDERER_RUNTIME_SCHEMA_SURFACE,
...
const surfaceTypes = RENDERER_RUNTIME_SCHEMA_SURFACE.map((entry) => entry.type);
...
describe('contracts/renderers surface integrity', () => {
  it.skipIf(UPDATE_MODE)('manifest dataTypes equal the package schema surface', () => {
    ...
    expect(manifest.contract).toBe('epoch/renderers');
    expect(manifest.contractVersion).toBe(RENDERER_CONTRACT_VERSION);
    expect(manifest.protocolVersion).toBe(RENDERER_PROTOCOL_VERSION);
    expect(manifest.documentKinds).toEqual([...RENDERER_DOCUMENT_KINDS]);
