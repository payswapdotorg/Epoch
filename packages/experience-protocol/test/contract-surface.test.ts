...
import {
  EXPERIENCE_PROTOCOL_SCHEMA_SURFACE,
...
const surfaceTypes = EXPERIENCE_PROTOCOL_SCHEMA_SURFACE.map((entry) => entry.type);
...
describe('contracts/experience surface integrity', () => {
  it.skipIf(UPDATE_MODE)('manifest dataTypes equal the package schema surface', () => {
    ...
    expect(manifest.contract).toBe('epoch/experience');
    expect(manifest.contractVersion).toBe(EXPERIENCE_CONTRACT_VERSION);
    expect(manifest.protocolVersion).toBe(EXPERIENCE_PROTOCOL_VERSION);
    expect(manifest.documentKinds).toEqual([...EXPERIENCE_DOCUMENT_KINDS]);
